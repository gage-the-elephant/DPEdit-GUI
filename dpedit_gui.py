...
class DisplayManager(tk.Frame):
    ...
    def __init__(self, parent, saved):
        ...
        # Create header
        self.__header = tk.Label(self.__sidebar_top, text='No display selected', font=('Segoe UI', 20, 'normal'),
                                 foreground='darkgrey')
        ...
        self.__input_x = tk.Entry(self.__sidebar_top, textvariable=self.__ui_xy_vals[0],
                                  font=('Segoe UI', 12, 'normal'), validate='key', validatecommand=self.__vcmd,
                                  state=tk.DISABLED)
        ...
        self.__input_y = tk.Entry(self.__sidebar_top, textvariable=self.__ui_xy_vals[1],
                                  font=('Segoe UI', 12, 'normal'), validate='key', validatecommand=self.__vcmd,
                                  state=tk.DISABLED)
        ...
        # Create buttons
        self.__btn_apply = tk.Button(self.__sidebar_bottom, text='Apply', command=self.apply,
                                     font=('Segoe UI', 12, 'normal'), state=tk.DISABLED)
        self.__btn_apply.pack(fill=tk.X, side=tk.BOTTOM, padx=8, pady=(2, 8))

        self.__btn_reset = tk.Button(self.__sidebar_bottom, text='Reset', command=self.reset,
                                     font=('Segoe UI', 12, 'normal'), state=tk.DISABLED)
    ...
    def undo(self):
        ...
        if len(self.__changes) > 1:
            ...
            for display in self.display_data:
                ...
                diff_xy = (
                    (self.display_coords[display['index'] - 1][0] // 10) -
                    self.__canvas.bbox(self.__drag_data['items'][0])[
                        0] - 1,
                    (self.display_coords[display['index'] - 1][1] // 10) -
                    self.__canvas.bbox(self.__drag_data['items'][0])[
                        1] - 1)
        ...
        if str(self.__btn_reset['state']) == 'normal':
            ...
            if prompt:
                ...
                for display in self.display_data:
                    ...
                    diff_xy = ((display['x'] // 10) - self.__canvas.bbox(self.__drag_data['items'][0])[0] - 1,
                               (display['y'] // 10) - self.__canvas.bbox(self.__drag_data['items'][0])[1] - 1)
                    self.__move_current_display(*diff_xy)
    ...
    def sync_canvas(self):
        ...
        for display in self.display_data:
            ...
            diff_xy = ((self.display_coords[int(self.__canvas.gettags(self.__drag_data['items'][0])[0]) - 1][0] // 10) -
                       self.__canvas.bbox(self.__drag_data['items'][0])[0] - 1,
                       (self.display_coords[int(self.__canvas.gettags(self.__drag_data['items'][0])[0]) - 1][1] // 10) -
                       self.__canvas.bbox(self.__drag_data['items'][0])[1] - 1)
            self.__move_current_display(*diff_xy)
    ...
    # ---------------------------------------
    def __sync_entry(self, var: object, index: object, mode: object) -> object:
        ...
        try:
            ...
            if not self.__drag_data['items'][0] and not 'static' in self.__canvas.gettags(self.__selected_display):
                diff_xy = ((self.__ui_xy_vals[0].get() // 10) - self.__canvas.bbox(self.__selected_display)[0] - 1,
                           (self.__ui_xy_vals[1].get() // 10) - self.__canvas.bbox(self.__selected_display)[1] - 1)
                ...
                self.display_coords[int(self.__canvas.gettags(self.__selected_display)[0]) - 1][0] = self.__ui_xy_vals[
                    0].get()
                self.display_coords[int(self.__canvas.gettags(self.__selected_display)[0]) - 1][1] = self.__ui_xy_vals[
                    1].get()
    ...
    def __drag_start(self, event):
        ...
        for item in self.__canvas.find_overlapping(self.__canvas.canvasx(event.x) - 2,
                                                   self.__canvas.canvasy(event.y) - 2,
                                                   self.__canvas.canvasx(event.x) + 2,
                                                   self.__canvas.canvasy(event.y) + 2):
        ...
        if self.__drag_data['items'][0] and self.__drag_data['dragged']:
            ...
            while True:
                ...
                for item in self.__canvas.find_overlapping(self.__canvas.bbox(self.__drag_data['items'][0])[0] + 2,
                    ...
                    if ('primary' in self.__canvas.gettags(item) or 'secondary' in self.__canvas.gettags(
                            item)) and item not in self.__drag_data['items']:
                        overlap_pos = self.__canvas.bbox(item)
                        drag_pos = self.__canvas.bbox(self.__drag_data['items'][0])
                        overlap_med = (overlap_pos[0] + (overlap_pos[2] - overlap_pos[0]) // 2,
                                       overlap_pos[1] + (overlap_pos[3] - overlap_pos[1]) // 2)
                        drag_med = (
                            drag_pos[0] + (drag_pos[2] - drag_pos[0]) // 2,
                            drag_pos[1] + (drag_pos[3] - drag_pos[1]) // 2)
            ...
            self.display_coords[int(self.__canvas.gettags(self.__drag_data['items'][0])[0]) - 1][0] = (
                                                                                                              self.__canvas.bbox(
                                                                                                                  self.__drag_data[
                                                                                                                      'items'][
                                                                                                                      0])[
                                                                                                                  0] + 1) * 10
            self.display_coords[int(self.__canvas.gettags(self.__drag_data['items'][0])[0]) - 1][1] = (
                                                                                                              self.__canvas.bbox(
                                                                                                                  self.__drag_data[
                                                                                                                      'items'][
                                                                                                                      0])[
            ...
            # Update sidebar data
            self.__ui_xy_vals[0].set(
                self.display_coords[int(self.__canvas.gettags(self.__drag_data['items'][0])[0]) - 1][0])
            self.__ui_xy_vals[1].set(
                self.display_coords[int(self.__canvas.gettags(self.__drag_data['items'][0])[0]) - 1][1])
    ...
    def __update_selection(self):
        ...
        # Update sidebar data
        self.__header.configure(foreground='black',
                                text=('Display #' + str(int(self.__canvas.gettags(self.__selected_display)[0]))))
    ...
    def __create_display(self, x, y, w, h, taglist):
        ...
        self.__canvas.create_text(x + w / 2, y + h / 2, text=taglist[0], anchor='center',
                                  font=('Segoe UI', 48, 'normal'), fill='white', tags=(taglist[0], 'disp_lbl', 'fg'))
        self.__canvas.create_text(x + w - 6, y - 6, text=('*' if 'primary' in taglist else ''), anchor='ne',
                                  font=('Segoe UI', 24, 'normal'), fill='white', tags=(taglist[0], 'prim_lbl', 'fg'))
    ...
    def __enum_display_devices(self):
        ...
        for display in self.display_data:
            x1, y1, x2, y2 = min(x1, display['x']), min(y1, display['y']), max(x2,
                                                                               display['x'] + display['width']), max(y2,
                                                                                                                     display[
                                                                                                                         'y'] +
                                                                                                                     display[
                                                                                                                         'height'])
        ...
        # Insert displays
        for display in self.display_data:
            self.__create_display(display['x'], display['y'], display['width'], display['height'], (
                ' ' + str(display['index']) + ' ', 'primary' if display['primary'] else 'secondary',
                'static' if display['primary'] else 'draggable'))
            self.display_coords.append([display['x'], display['y']])
...
def load_profile(args=None):
    try:
        global profile_path, is_saved
        with open(filedialog.askopenfilename(initialdir='%userprofile%\Documents',
                                             filetypes=(('DPEdit-GUI Config Files', '*.dgc'), ('All Files', '*.*'))),
                  'r') as file:
            assert file
...
def save_profile_as(args=None):
    try:
        global profile_path, is_saved
        with filedialog.asksaveasfile(mode='w', initialdir='%userprofile%\Documents', defaultextension='.dgc',
                                      initialfile=profile_path,
                                      filetypes=(('DPEdit-GUI Config Files', '*.dgc'), ('All Files', '*.*'))) as file:
            assert file
            data = str(disp_man.display_coords)
            file.write(data)
            profile_path = file.name
...
def quit_app(args=None):
    if not is_saved[0]:
        response = messagebox.askyesnocancel(
            message='The display profile has not been saved.\nWould you like to save now?', title='Save Profile')
        if response:
            save_profile()
        elif response == None:
            return
